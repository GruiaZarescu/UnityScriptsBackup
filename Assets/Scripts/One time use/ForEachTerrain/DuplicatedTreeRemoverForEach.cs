...
public class DuplicatedTreeRemoverForEach : MonoBehaviour
{
    ...
    [SerializeField] private float minDistance6 = 1f;
    ...
    private void RunPass(Vector2 offset)
    {
        ...
        for (int treeIndex=0;treeIndex<terrains.Length;treeIndex++)
        {
            Terrain _terrain = terrains[treeIndex];
            currentTerrainIndex = treeIndex;
    ...
    {
        Vector3 size = terrains[currentTerrainIndex].terrainData.size;
        return Vector3.Scale(tree.position, size) + terrains[currentTerrainIndex].transform.position;
    }
