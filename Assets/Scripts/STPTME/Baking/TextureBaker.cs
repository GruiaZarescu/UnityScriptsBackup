...
public class TextureBaker
{
        ...
        TextureBakeSettings settings)
    {
        ...
        // Explicitly release texture references held by TerrainLayer objects to allow GC
        // These textures are kept alive by the TerrainLayer array even after baking
        for (int i = 0; i < layers.Length; i++)
        {
            if (layers[i] != null)
            {
                // Clear texture references - this allows Unity to unload them
                layers[i].diffuseTexture = null;
